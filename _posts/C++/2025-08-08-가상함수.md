...
## 개요
개발을 진행하다보면 관련된 여러 객체를 하나의 인터페이스로 관리해야 하는 상황을 마주하게 된다. 예를 들어, 게임 내 전사, 궁수, 마법사 캐릭터가 있을 때 똑같이 '공격'이라는 공통된 행위를 하지만, 각자만의 방식으로 이를 수행한다. 만약 모든 캐릭터를 `Character` 타입의 포인터 배열로 관리하면서, 각 캐릭터의 실제 클래스에 맞는 `Attack` 함수를 호출하고 싶다면 어떻게 해야할까?
...
결과를 확인해보면, 우려했던 대로 `Animal`의 소멸자가 호출되는 것을 알 수 있다. 만약 `Dog` 클래스 내에 동적으로 메모리를 할당했다면, 이 메모리를 해제되지 못하고 그래도 **메모리 누수(Memory Leak)**가 발생했을 것이다.
