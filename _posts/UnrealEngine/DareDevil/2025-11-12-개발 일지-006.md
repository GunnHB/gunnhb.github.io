---
title: "[UnrealEngine][DareDevil] 개발 일지 - 006"
description: 2025년 11월 12일 개발 일지
...
- 파쿠르 구현을 위한 장애물 감지 및 분석 로직 구현

## 🤔 작업 내용

파쿠르를 구현하기 위해서는 다음과 같은 로직의 구현이 필요하다.

- 벽/장애물 감지
- 감지를 통해 얻은 정보 분석

우선 SphereTrace를 통해 액터 앞에 장애물을 감지하도록 했다.
...
플레이어의 종아리 부근과 가슴팍에서 충돌 체크를 진행하면서 장애물의 정보를 얻기 위한 초석을 다졌다.

장애물이 있는 것을 확인했으니 이제 해당 장애물의 높이와 폭을 알아야 했다. 우선 장애물의 높이를 얻기 전, 해당 장애물이 **넘어설 수 있는지를 판단**하기 위해 `Line Trace`를 이용했다.
...
충돌 정보의 `Normal`의 반대 방향으로 `Line Trace`를 발사한다. 여기서 중요한 점은 파쿠르 가능 최대 높이에서부터 충돌한 `Sphere Trace`의 `Impact` 위치까지 일정 간격으로 발사한다. 반복문을 통해 `Line Trace`를 발사하면서 만약 충돌이 발생하게 되면 바로 `break`문을 실행한다.

![](/assets/img/post/Daredevil/Parkour/line_trace.png){: width="600" height="400" }
_파란 선으로 표현됐다_

위 로직으로 `Line Trace`를 실행하면 장애물의 대략적인 높이를 확인하면서 어떤 액션을 취할 것인지 파악할 수 있다. 하지만 **정확한 높이와 폭을 측정**해야 한다.
...
정확한 높이를 측정하기 위해서 이번에는 `Sphere Trace`를 활용했다. `Line Trace`를 통해 얻은 충돌 지점에서부터 일정 개수의 `Sphere Trace`를 장애물 방향으로 배치한다.
...
여기서 첫 `Sphere Trace`가 충돌하는 지점이 바로 **첫 모서리 지점**이다. 그 뒤 반복문을 돌면서 충돌이 일어날 경우 마지막 모서리 정보로 갱신해준다. 만약 충돌이 일어나지 않을 경우, 해당 `Sphere Trace`는 장애물에 충돌되지 않았으므로 갱신하지 않는다.

하지만 이렇게 되면 마지막 충돌 지점은 정확한 모서리가 아니게 된다. 그렇기에 `Sphere Trace`가 충돌되지 않는 시점에 (마지막 충돌 위치보다 뒤쪽 - 마지막 충돌 위치)만큼 `Sphere Trace`를 수행하여 정확한 모서리 지점을 확인한다.

![](/assets/img/post/Daredevil/Parkour/sphere_trace_depth.png){: width="600" height="400" }
_DrawDebugSphere를 이용해 모서리 지점을 표시했다_

이렇게 장애물의 높이와 폭을 구할 수 있게 됐다. 이를 이용해 장애물에 따른 파쿠르 액션을 지정하여 수행할 수 있을 것이다.

## 💡 새롭게 배운 점

- 여러 트레이스를 통한 오브젝트 정보 추출
