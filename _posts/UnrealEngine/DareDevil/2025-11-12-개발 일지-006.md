...
위 로직으로 `Line Trace`를 실행하면 장애물의 대략적인 높이를 확인하면서 어떤 액션을 취할 것인지 파악할 수 있다.이제  **정확한 높이와 폭을 측정**해야 한다.
...
높이를 측정하기 위해 다시 `Sphere Trace`를 활용했다. `Line Trace`를 통해 얻은 충돌 지점에서부터 일정 개수의 `Sphere Trace`를 장애물 방향으로 배치한다.
